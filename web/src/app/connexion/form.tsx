...
//
// L'ordre d'affichage suit ce qui marche vraiment. Tant qu'aucun fournisseur
// OAuth n'est activé côté Supabase, mettre quatre boutons « à fixer » en tête
// et reléguer l'e-mail en bas revient à présenter d'abord les portes fermées :
// on inverse, et le seul chemin ouvert passe devant.
...
export function ConnexionForm({ linkExpired, lang }: { linkExpired: boolean; lang: Language }) {
  ...
  const hasOtherMethods = providers.length > 0 || !emailCode;
  ...
  // Les fournisseurs OAuth et le code à 6 chiffres : actifs, ou en attente.
  const otherMethods = hasOtherMethods ? (
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-16">
      ...
      <p className="mt-2 text-muted">{hasLive ? tr.subtitle : tr.subtitleLinkOnly}</p>

      {linkExpired && !sent && <Notice>{tr.previousFailed}</Notice>}
      {error && <Notice>{error}</Notice>}

      {hasLive ? (
        // Au moins un fournisseur marche : il garde la tête d'affiche.
        <>
          <div className="mt-8">{otherMethods}</div>
          {otherMethods && <Divider label={tr.orByEmail} />}
          {emailBlock}
        </>
      ) : (
        // Aucun fournisseur câblé : l'e-mail d'abord, le reste en pense-bête.
        <>
          <div className="mt-8">{emailBlock}</div>
          {otherMethods && (
            <>
              <Divider label={tr.notYetLabel} />
              {otherMethods}
            </>
          )}
        </>
